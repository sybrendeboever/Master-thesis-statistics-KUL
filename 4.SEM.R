...
for (i in 1:length(mice.imp)){
  ...
  mice.imp[[i]]$PL = ifelse(mice.imp[[i]]$Q15==" 1",1,0)
  mice.imp[[i]]$CI = ifelse(mice.imp[[i]]$Q15==" 2",1,0)
  mice.imp[[i]]$OE = ifelse(mice.imp[[i]]$Q15==" 3",1,0)
  mice.imp[[i]]$idk = ifelse(mice.imp[[i]]$Q15==" 0",1,0)
  mice.imp[[i]]$PL_OE = ifelse(mice.imp[[i]]$Q15==" 6",1,0)
  mice.imp[[i]]$PL_CI = ifelse(mice.imp[[i]]$Q15==" 7",1,0)
  mice.imp[[i]]$OE_CI = ifelse(mice.imp[[i]]$Q15==" 8",1,0) # Ref = PL+OE+CI
  # Combined
...
# --------------------------------------- MIMIC model ---------------------------------------------
# For not-imputed data:
# ---------------------
# Background variables
# 1. Fase
test$Fase1 = ifelse(test$Fase==1,1,0)
test$Fase1[which(is.na(test$Fase))] = NA
test$Fase2 = ifelse(test$Fase==2,1,0)
test$Fase2[which(is.na(test$Fase))] = NA
test$Fase3 = ifelse(test$Fase==3,1,0)
test$Fase3[which(is.na(test$Fase))] = NA
test$Fase4 = ifelse(test$Fase==4,1,0) # ref = Fase5 (=MA)
test$Fase4[which(is.na(test$Fase))] = NA
# 2. Q5: parents
test$Parents = ifelse(test$Q5==1,1,0) # ref= parents not engineer
test$Parents[which(is.na(test$Q5))] = NA
# 3. Gender
test$sex = ifelse(test$Gender=="M",1,0) # ref = female
test$sex[is.na(test$Gender)] = NA
# 4. Considering another field than engineering? (Q31)
test$indec = 0
test$indec[test$Q31=="2"] = 1 # "yes" (= 'idecisive')
test$indec[which(is.na(test$Q31))] = NA
test$quest = 0
test$quest[test$Q31=="1"] = 1 # sometimes (= 'questionable')
test$quest[which(is.na(test$Q31))] = NA # ref = "nee" = 'denkt niet aan een andere loopbaan dan ingenieur'
## Professional role preference
# Separate
test$PL = ifelse(test$Q15_1=="1",1,0)
test$CI = ifelse(test$Q15_2=="1",1,0)
test$OE = ifelse(test$Q15_3=="1",1,0)
test$idk = ifelse(test$Q15_4=="1",1,0)
test$PL_OE = ifelse(test$Q15_6=="1",1,0)
test$PL_CI = ifelse(test$Q15_7=="1",1,0)
test$OE_CI = ifelse(test$Q15_8=="1",1,0) # Ref = PL+OE+CI
# Combined
test$OE_overall =0
test$OE_overall[test$Q15_3=="1" | test$Q15_5=="1" | test$Q15_6=="1" | test$Q15_8=="1"] = 1
test$CI_overall = 0
test$CI_overall[test$Q15_2=="1" | test$Q15_5=="1" | test$Q15_7=="1" | test$Q15_8=="1"] = 1 # Ref = PL_overall
test$Q15[test$Q15_1=="1"|test$Q15_2=="1"|test$Q15_3=="1"|test$Q15_5=="1"|test$Q15_6=="1"|test$Q15_7=="1"|test$Q15_8=="1"] = "1"
test$Q15[test$Q15_4 == "1"] = "0" # idk = ref (Opm: deze code zorgt er voor dat rekening wordt gehouden met de originele NAs in de dataset)
# Know vocational interest: yes/no
test$Q15[test$Q15_1=="1"|test$Q15_2=="1"|test$Q15_3=="1"|test$Q15_5=="1"|test$Q15_6=="1"|test$Q15_7=="1"|test$Q15_8=="1"] = "1"
test$Q15[test$Q15_4 == "1"] = "0" # idk = ref (Opm: deze code zorgt er voor dat rekining wordt gehouden met de originele NAs in de dataset)
...
model = ' # Measurement model
        ...
        # Structural model
          # For not-imputed data:
        #     |------------FASE------------|-GNDR-|----Q5--|-----Q31-------|-Q15
        # CE ~ Fase1 + Fase2 + Fase3 + Fase4 + sex + Parents + indec + quest + Q15
        # CFC ~ Fase1 + Fase2 + Fase3 + Fase4 + sex + Parents + indec + quest + Q15
        # CC ~ Fase1 + Fase2 + Fase3 + Fase4 + sex + Parents + indec + quest + Q15
        # RU ~ Fase1 + Fase2 + Fase3 + Fase4 + sex + Parents + indec + quest + Q15
        # CU ~ Fase1 + Fase2 + Fase3 + Fase4 + sex + Parents + indec + quest + Q15
        
          # For IMPUTED data:
        CE ~ Fase1 + Fase2 + Fase3 + Fase4 + sex + Parents + indec + quest + PL + CI + OE + PL_CI + PL_OE + OE_CI +idk + Migration + Migration_other
        CFC ~ Fase1 + Fase2 + Fase3 + Fase4 + sex + Parents + indec + quest +PL + CI + OE + PL_CI + PL_OE + OE_CI +idk + Migration + Migration_other
        CC ~ Fase1 + Fase2 + Fase3 + Fase4 + sex + Parents + indec + quest + PL + CI + OE + PL_CI + PL_OE + OE_CI +idk + Migration + Migration_other
        RU ~ Fase1 + Fase2 + Fase3 + Fase4 + sex + Parents + indec + quest + PL + CI + OE + PL_CI + PL_OE + OE_CI +idk + Migration + Migration_other
        CU ~ Fase1 + Fase2 + Fase3 + Fase4 + sex + Parents + indec + quest + PL + CI + OE + PL_CI + PL_OE + OE_CI +idk + Migration + Migration_other
'
# Wrong fit measures for MI: https://github.com/simsem/semTools/issues/29
...
# Fit measure per run are larger than pooled fit measures, which appear to be wrong
# (the output of runMI says "Rubin's rules were used to pool 
# (1) point and SE estimates, (2) degrees of freedom for each parameter's t-test and CI
# which does not mention the fit measures (unles = point estimate), suggesting these are not based on Rubin)
# https://groups.google.com/g/lavaan/c/WM2Ynmatsmk/m/kAKH8yZ8AwAJ:
# "semTools calculates CFI based on the (single) pooled chi-squared statistic -- the same one you see for the test of model fit"
df = c()
for (i in 1:length(mice.imp)){
